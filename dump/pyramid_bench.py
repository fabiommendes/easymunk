...
class PyramidDemo:
    ...
    def __init__(self):
        ...
        ### ground
        body = pymunk.Body()
        shape = pymunk.Segment(body, (50, 100), (550, 100), 0.0)
        shape.friction = 1.0
    ...
    def draw(self):
        ...
        for shape in self.space.shapes:
            if shape.body.is_static:
                body = shape.body
                pv1 = self.flipyv(
                    body.position + shape.a.cpvrotate(body.rotation_vector)
                )
                pv2 = self.flipyv(
                    body.position + shape.b.cpvrotate(body.rotation_vector)
                )
...
def main():
    t = timeit.Timer(
        "demo.run()",
        "gc.enable(); from __main__ import PyramidDemo;demo = PyramidDemo()",
    )
