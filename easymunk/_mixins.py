...
class MakeShapeMixin:
    ...
    def _new_shape_post(self, shape, **kwargs):
        for k, v in kwargs.items():
            setattr(self, k, v)
        return shape

    def new_circle_shape(self, radius, offset=(0, 0), **kwargs):
        shape = self._circle_class(self, radius, offset)
        return self._new_shape_post(shape, **kwargs)

    def new_segment_shape(self, a, b, radius=1, *, world=False, **kwargs):
        if world:
            a, b = map(self.world_to_local, (a, b))
        shape = self._segment_class(self, a, b, radius)
        return self._new_shape_post(shape, **kwargs)

    def new_poly_shape(self, vertices, radius=0, transform=None, world=False, **kwargs):
        if world:
            vertices = list(map(self.world_to_local, vertices))
        shape = self._poly_class(self, vertices, transform, radius)
        return self._new_shape_post(shape, **kwargs)

    def new_rect_shape(self, width, height, offset=(0, 0), **kwargs):
        w = width / 2
        h = height / 2
        x, y = offset
        vs = [(x - w, y - h), (x + w, y - h), (x + w, y + h), (x - w, y + h)]
        return self.new_poly_shape(vs, **kwargs)

    def new_regular_poly_shape(self, n, size, offset=(0, 0), rotation=0, **kwargs):
        u = Vec2d(size, 0).rotated(rotation)
        origin = Vec2d(offset)
        delta = 2 * pi / n
        vs = [origin + u.rotated(delta * i) for i in range(n)]
        return self.new_poly_shape(vs, **kwargs)
